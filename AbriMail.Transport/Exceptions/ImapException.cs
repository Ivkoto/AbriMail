namespace AbriMail.Transport
{
  /// <summary>
  /// Exception thrown when IMAP operations fail.
  /// </summary>
  public class ImapException : Exception
  {
    /// <summary>
    /// Initializes a new instance of the <see cref="ImapException"/> class.
    /// </summary>
    public ImapException() : base() { }

    /// <summary>
    /// Initializes a new instance of the <see cref="ImapException"/> class with a specified error message.
    /// </summary>
    /// <param name="message">The error message that explains the reason for the exception.</param>
    public ImapException(string message) : base(message) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="ImapException"/> class with a specified 
    /// error message and a reference to the inner exception that is the cause of this exception.
    /// </summary>
    /// <param name="message">The error message that explains the reason for the exception.</param>
    /// <param name="innerException">The exception that is the cause of the current exception.</param>
    public ImapException(string message, Exception innerException) : base(message, innerException) { }

    /// <summary>
    /// IMAP server response that caused the exception.
    /// </summary>
    public string? ServerResponse { get; set; }

    /// <summary>
    /// IMAP command that failed.
    /// </summary>
    public string? FailedCommand { get; set; }
  }
}
